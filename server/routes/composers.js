...
router.post('/', [auth, admin], async (req, res) => {
    ...
    const composer = new Composer(_.pick(req.body, ['name', 'date', 'nationality', 'website','biography']));
    await composer.save();
...
router.put('/:id', [auth, admin], async (req, res) => {
    ...
    if (error) return res.status(400).send(error.details[0].message);
//
    const composer = await Composer.findByIdAndUpdate(req.params.id, _.pick(req.body, ['name', 'date', 'nationality', 'website', 'biography']), { new: true });
